...
void setup() {
  ...
  Serial3.print("************BIENVENIDO AL REGISTRO DEL VEHÍCULO AUTOMATIZADO************\n En esta pantalla se le mostrarán parámetros de tiempo y estatus de los recorridos que el vehículo realice \n\n");
  Serial3.print("************INSTRUCCIONES DE USO*************\n 1.para la carga y descarga de materiales el vehiculo se tiene que encontar en la estación con las guías blancas.\n ");
  Serial3.print("2. Para iniciar un recorrido se debe de cargar primeramente la carga y después cambiar el color de la estación de carga \n"); 
  Serial3.print("3. El vehículo se detendrá cuando ambos sensores detecten color blanco.");
  Serial3.print("\n");
  Serial3.print("\n");
  Serial3.print("\n");
  Serial3.print("\n");
...
Serial.print(val);
 ...
 if((t>0)||((t>0)&&((LsensorU == LOW && LsensorD == LOW)||kk==0))){
   Serial3.print("Se dejó de detectar peso \n");
   Serial3.print("Tiempo total de recorrido: ");
   Serial3.println(t);
  ...
  while((val==HIGH)&&(kk=1)){
    ...
    starte=1;
    LsensorU = digitalRead(SensorUno);//A5 Sensor derecha cable naranja
    LsensorD = digitalRead(SensorDos);
    if((LsensorU ==LOW && LsensorD == LOW)){
      kk=3;
      }
 ...
 /*estados de movimiento*/
  ...
  LsensorD = digitalRead(SensorDos);//A4 Sensor izquierda cable café
   if (LsensorU == HIGH && LsensorD == HIGH) {
    MotorUno.setSpeed(650);
    MotorDos.setSpeed(650);
    MotorTres.setSpeed(650);
  ...
  if (LsensorU == HIGH && LsensorD == LOW) {
    ...
    MotorUno.run(FORWARD); //hacia atrás llanta DAdelante
    MotorDos.run(BACKWARD);//hacia delante llanta IAdelante
    MotorTres.run(FORWARD); // hacia atrás DTrasera
    MotorCuatro.run(BACKWARD);// hacia delante ITrasera
  ...
  if (LsensorU == LOW && LsensorD == HIGH) {
   // para que se muva a la izquierda necesita detectar que el de la derecha esté en blanco
    ...
    MotorDos.run(BACKWARD);//hacia atrás llanta IAdelante
    MotorTres.run(BACKWARD); // hacia delante DTrasera
    MotorCuatro.run(FORWARD);// hacia atrás ITrasera  
    }
